...
def welcome():
    ...
    #window.geometry('700x300+500+300')
    window.resizable(0,0)



    chk1 = Label(window, text="1.已安装Python3.x版本").grid(column=0, row=0)
    chk2 = Label(window, text="2.已安装pygame,PIL,zipfile,readfile,pydub,eyed3库").grid(column=0, row=1)
    chk3 = Label(window, text="3.已安装Exo与Saira字体").grid(column=0, row=2)
    chk3 = Label(window, text="4.环境为macOS，若不是请修改路径").grid(column=0, row=3)



    yesbutton = Button(window,text="确定",command=window.destroy).grid(column=0, row=4)
    nobutton  = Button(window,text="取消",command=sys.exit).grid(column=1, row=4)
...
def choose():
    ...
    font_EN=pygame.freetype.Font(r"/Users/zhangzhiqin/Library/Fonts/Saira-Medium.ttf",18*1.3)#设置字体
    #背景图片
            ...
            if event.type==pygame.MOUSEBUTTONDOWN:
                if event.button==1:
                    ...
                    if event.pos[0]>=button_x                      and event.pos[0]<=button_x+button_width                    and event.pos[1]>=button_y and event.pos[1]<=button_y+button_height:
                        ...
                        #判断鼠标点击位置是否在选歌按钮2上 BUG：没有判断选歌按钮2的位置
                        ...
                        #判断鼠标点击位置是否在选歌按钮3上 BUG：没有判断选歌按钮3的位置 没有按钮3
