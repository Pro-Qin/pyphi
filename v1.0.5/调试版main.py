...
while 1:
    ...
    SongsName=f2.render_to(screen,[30,503],info_data['other'][24],fgcolor=WHITE,size=21)      #歌曲名
    SongsLevel=f1.render_to(screen,[865,507],info_data['other'][25],fgcolor=WHITE,size=18)    #歌曲等级
    ...
    #----------------------------------------------------调试显示区
    try:
        otime=f1.render_to(screen,[0,5],'{}/{}'.format(time.strftime("%M:%S",time.localtime(less)),songlengthstr),fgcolor=WHITE,size=12)
    except TypeError:
        otime=f1.render_to(screen,[0,5],'{}/{}'.format(songlengthstr,songlengthstr),fgcolor=WHITE,size=12)
