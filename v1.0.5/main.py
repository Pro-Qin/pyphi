...
raise NoError('方便程序测试使用的错误，在代码里删掉即可.')
...
while 1:
    ...
    SongsName=f2.render_to(screen,[30,503],info_data['other'][24],fgcolor=(255,255,255),size=21)      #歌曲名
    SongsLevel=f1.render_to(screen,[865,507],info_data['other'][25],fgcolor=(255,255,255),size=18)    #歌曲等级
    mark=f1.render_to(screen,[809,25],str(nummark),fgcolor=(255,255,255),size=28)               #分数
    try:
        otime=f1.render_to(screen,[0,5],'{}/{}'.format(time.strftime("%M:%S",time.localtime(less)),songlengthstr),fgcolor=(255,255,255),size=12)
    except TypeError:
        otime=f1.render_to(screen,[0,5],'{}/{}'.format(songlengthstr,songlengthstr),fgcolor=(255,255,255),size=12)
